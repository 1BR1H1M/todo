$("#form-login-user").submit(function( event ) {
	...
	$.ajax({
		type: 		"POST",
		url: 		base_url+"/login/loginValidate",
		data: 		parametros,
		dataType: 	"JSON",
		...
		success: function(datos){
			...
			if (datos.tipo == "success") {
				...
				$("#div-cnt-login").html('<div class="col-12 text-center"><h4><div class="spinner-own sm-br spin-3x" role="status"><span class="sr-only">Loading...</span></div> <br>Redireccionando...</h4></div>');
				setTimeout(function(){
					$(window).attr('location', base_url+'dashboard');
				}, 3000);
			}
		},
